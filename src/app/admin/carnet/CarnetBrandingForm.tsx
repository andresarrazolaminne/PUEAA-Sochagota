...
import { saveCarnetBrandingAction } from "./actions";
...
function PreviewImage({ src, alt }: { src: string; alt: string }) {
  ...
  const className =
    "max-h-40 w-auto max-w-full rounded border border-[#243d30] bg-[#0d1512] object-contain p-2";
...
export function CarnetBrandingForm({ initialLogoSrc, initialCaption }: Props) {
  ...
  return (
    <form action={saveCarnetBrandingAction} className="mt-6 flex max-w-xl flex-col gap-4">
      <div className="rounded-md border border-[#243d30] bg-[#0a100d] p-4">
        ...
        <div className="mt-3 flex min-h-[120px] items-center justify-center">
          <PreviewImage src={displaySrc} alt="Logo del carné" />
        </div>
        {filePreview ? (
          <p className="mt-2 text-xs text-[#7aab8c]">
            Vista previa del archivo seleccionado. Quita el archivo para volver a ver el logo guardado.
          </p>
        ) : (
          <p className="mt-2 text-xs text-[#7aab8c]">Logo que verán los jugadores en el tablero.</p>
        )}
      </div>
      ...
      <label className="flex flex-col gap-1.5">
        ...
        <input
          ...
          placeholder="/carnet-cazadores-gastos-fantasma.png"
        />
      </label>

      <label className="flex flex-col gap-1.5">
        <span className="font-mono text-[10px] text-[#6a9c80]">Leyenda bajo el logo</span>
        <input
          name="caption"
          ...
          placeholder="Texto breve visible en el carné"
        />
      </label>
