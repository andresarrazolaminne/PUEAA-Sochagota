...
export function getSiteLogoUploadDir(): string {
  return path.join(resolveUploadDir(), "site-logo");
}
...
export async function saveSiteLogoUpload(
  ...
  const dir = getSiteLogoUploadDir();
  ...
  return { ok: true, publicPath: publicSiteLogoUploadUrl(filename) };
}
